...
variable "cluster_name" {
  description = "클러스터 이름 (리소스 prefix로 사용)"
  type        = string
}

variable "environment" {
  description = "환경 (demo / qa)"
  type        = string
  validation {
    condition     = contains(["demo", "qa"], var.environment)
    error_message = "environment는 'demo' 또는 'qa'만 가능합니다."
  }
}
...
variable "secloudit_version" {
  ...
  type        = string
  validation {
    condition     = contains(["v1.5", "v2.0", "v2.3", "v2.6"], var.secloudit_version)
    error_message = "secloudit_version은 'v1.5', 'v2.0', 'v2.3', 'v2.6' 중 하나여야 합니다."
  }
}
...
variable "cluster_type" {
  ...
  default     = "separated"
  validation {
    condition     = contains(["all-in-one", "separated"], var.cluster_type)
    error_message = "cluster_type은 'all-in-one' 또는 'separated'만 가능합니다."
  }
}
